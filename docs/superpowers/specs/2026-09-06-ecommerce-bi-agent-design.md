...
日期：2026-09-06；2026-09-09修订前后端边界。状态：设计已确认；仓库已有Streamlit原型，本次规划的React/FastAPI拆分尚未实施；已按用户授权完成少量商家接口只读抽样，尚未完成财务对账。
...
- 启动设计时工作区根目录只有 `.env` 和参考项目；随后已按原计划形成Streamlit原型并提交到根Git仓库。当前修订从这套可运行代码迁移，不重新实现已验证的同步、指标、费用测算和provider适配。
...
| 路线 | 优点 | 代价 | 结论 |
| --- | --- | --- | --- |
| A. React聊天前端 + FastAPI后端 + 少量业务工具 + 参数化 SQL | 前后端可分别修改，产品形态贴近聊天应用；指标仍由后端统一控制 | 比Streamlit多一个前端构建环境 | **用户确认作为第一版** |
...
路线 A 的真实定位是“聊天界面承载、以业务工具查询数据库的对话 Agent”：模型选择工具、理解时间与维度、衔接追问、解释已计算结果，查询由可验证的后端代码执行。面试时不将其包装成已实现的通用 Text2SQL。

### 具体技术选型

| 部分 | 第一版选择 | 理由与边界 |
| --- | --- | --- |
| 后端语言与依赖 | Python 3.11 + uv | 同步、指标、Agent和API共用一套业务代码 |
| 前端语言与依赖 | Node.js 22 LTS + npm + React 19 + TypeScript + Vite | 聊天组件清晰，前后端可独立开发；不用Next.js增加第二个服务端 |
| 前端界面 | DeepSeek风格的单页聊天工作台，普通CSS | 左侧会话列表、中间消息流、底部固定输入框；不复制品牌标识，不引入组件库或状态管理库 |
| 后端 API | FastAPI + Uvicorn | 只提供身份、会话、消息流和健康检查；业务查询函数不直接公开为通用API |
...
| 数据调度 | 一个同步命令 + Windows 任务计划程序；Linux 部署用系统定时器 | 默认每小时一次、单实例执行，不放进FastAPI进程 |
| 结果展示 | React消息卡片、HTML表格、简单CSS条形/趋势展示 | 图表是消息附件；首版不引入图表库，复杂交互有真实需求时再加 |
| 业务指标与规则 | Python/SQL 常量映射 + 一份指标说明 Markdown | 少量表与指标直接给模型描述，不用向量检索 |
| 验证与观测 | 后端标准库 unittest + 前端构建/SSE解析检查 + 脱敏结构化日志 + 一份问答验收集 | 验证金钱计算、同步幂等、API归属和聊天事件；暂不建设独立观测平台 |
...
超时、认证失败、限流、工具参数格式错误向 Agent 返回一致的错误分类，保留脱敏的 provider、model 和请求标识供排查。沿用整个回答30秒的总预算；适配层与 SDK 不叠加各自独立的多轮重试。第一版不自动切换供应商，不做负载均衡或成本路由。浏览器与FastAPI之间使用SSE传递阶段、结果附件和最终消息；模型适配层首版仍使用完整响应，不承担token级流式拼装。
...
flowchart LR
    ...
    USER[经营者] --> WEB[React聊天前端]
    WEB -->|同源JSON + SSE| API[FastAPI]
    API --> CHAT[(会话与消息)]
    API --> AGENT[单 Agent 与参数校验]
...
一个仓库内分为前端和后端两个工程；开发时各自启动，生产时由同源反向代理提供静态前端和 `/api`，后端不直接暴露公网。FastAPI与同步命令共用后端业务包和一套PostgreSQL；这不是拆成多套业务微服务。
...
| API与会话 | 校验身份和会话归属、保存可见消息、把阶段/附件/最终消息转成SSE | 同源聊天请求 → 事件流 | 对话编排、PostgreSQL、反向代理身份 |
| React前端 | 新建/切换/改名/删除会话，发送问题，渲染消息、状态、指标附件和错误 | JSON/SSE → 聊天界面 | FastAPI；不接触数据库、ERP或模型密钥 |

拟议文件划分，不在本次生成空目录或占位代码：

```text
backend/
  pyproject.toml            # Python依赖和命令
  bi_agent/
    api.py                  # FastAPI路由、身份边界和SSE
    chats.py                # 会话/消息持久化与归属校验
  ...
  sql/001_init.sql          # 业务事实、会话表、约束、索引、视图和角色
  tests/                    # 后端业务、数据库、API和问答检查
frontend/
  package.json              # React/Vite依赖与脚本
  src/
    App.tsx                 # 页面状态和会话切换
    api.ts                  # JSON接口和SSE解析
    components/             # 侧栏、消息流、输入框、结果附件
    styles.css              # DeepSeek风格布局与响应式样式
docs/metrics.md             # 与查询实现对应的业务口径
```

前后端只共享API契约，不共享生成代码或包。`promotion.py` 尚无费用数据时只提供明确标注的情景计算；费用实绩导入代码和表随数据源一起增加，不提前搭插件系统。
...
浏览器只面向聊天领域，不暴露手动经营查询端点。第一版HTTP契约如下：

| 方法与路径 | 用途 |
| --- | --- |
| `GET /api/chats` | 当前身份的会话列表 |
| `POST /api/chats` | 新建空会话 |
| `GET /api/chats/{chat_id}/messages` | 当前身份拥有的可见消息和结果附件 |
| `POST /api/chats/{chat_id}/messages` | 发送一条用户消息，返回 `text/event-stream` |
| `PATCH /api/chats/{chat_id}` | 修改当前身份会话标题 |
| `DELETE /api/chats/{chat_id}` | 删除当前身份会话及消息 |
| `GET /api/health` | 进程存活状态，不泄露数据库、provider或同步细节 |

消息SSE依次允许 `status`、`artifact`、`message`、`error`、`done` 事件。`status`只表示理解问题、查询数据、组织回答等有限阶段；`artifact`是去除ERP标识并换成可见标签的指标卡/表格/趋势结果；`message`是最终持久化的完整助手消息。第一版不做token级增量，不承诺显示或保存模型私有推理。

### 聊天界面

视觉上参考DeepSeek聊天产品的克制布局，不复制名称、图标或颜色资产。桌面左侧约260px会话栏，提供新建会话、最近会话、改名和删除；主区消息内容最大宽度约800px，用户消息为轻量右侧气泡，助手正文采用开放排版。空会话中央显示一句能力说明和3个示例问题；底部输入框固定在内容区内，支持1～6行自动增高、Enter发送、Shift+Enter换行，运行中显示处理状态并禁用重复提交。

工具过程只显示有限状态，不展示思维链。结果附件紧跟助手消息，按数据形状选择指标卡、表格或简单趋势；每个附件都显示时间范围、数据截止和限制。错误在当前消息位置可读地呈现并允许重新发送。窄屏时会话栏变为可关闭抽屉，输入框和消息区域仍可键盘操作；使用语义HTML、可见焦点和 `aria-live`，不只用颜色表达状态。
...
6. 后端将确定性结果作为结构化附件与最终消息一起保存并发送；React渲染数字卡片、表格、比率和简单趋势，模型只解释变化与注意事项。

第一版工程默认上限：单次问题最多 4 次工具调用，一次参数修正机会，整个回答最多 30 秒；SQL `statement_timeout=5s`，明细/分组结果最多 500 行，单次日期范围最多 366 天且必须在已同步覆盖区间内。这些是应用预算，后续由真实数据量调优，不是快麦 API 限额。没有面向前端的手动查询入口；模型故障时本轮返回明确错误并保留已有会话和历史结果，不能编造回答。

### 会话持久化和并发

`app_chats`以服务端确认的身份和会话ID隔离，保存标题、当前已确认筛选和更新时间；`app_messages`保存用户/助手可见正文、结构化附件、状态和顺序。模型工具调用、ERP ID、完整供应商响应及私有推理不作为可见聊天历史长期保存。构造模型上下文时只取最近6个完整回合和当前筛选，避免无限增长。

同一会话同时只允许一个回答运行。后端对会话取得PostgreSQL advisory lock；竞争请求返回409，前端在事件流结束前禁用该会话输入。用户消息先持久化，成功后保存助手消息和附件；失败时保存可见错误消息，刷新页面后仍能理解发生了什么。客户端中途断开不删除已保存消息；完成结果若已产生仍可在刷新后加载。
...
- 建库时区分同步写入身份与应用身份；FastAPI应用身份只可读取授权报表视图并读写自身会话表，不能写经营事实表。浏览器永远不取得数据库身份，数据库不暴露公网。
- 内部工具也需要访问控制。生产优先由同源反向代理完成公司登录并向仅监听回环地址的FastAPI注入可信OIDC `sub`；代理必须覆盖而非转发浏览器同名身份头。没有认证设施时只允许localhost开发，不开放外部试用。
- 前端与 `/api` 生产同源，不启用宽泛CORS。所有写接口要求JSON与应用自定义请求头，并校验Origin，降低基于认证Cookie的跨站请求风险。
...
| 1. 确定性经营内核 | 同步入库 + 指标函数 | 通过后端测试可准确回答期间销售、趋势、排行；重复同步不变大，缺报与零业务可区分 |
| 2. 聊天后端 | FastAPI会话API + 可配置provider + 两个内部工具 + SSE | 会话归属不可越权；两个provider分别完成工具调用闭环；20题的日期、店铺、口径和结果符合人工答案 |
| 3. 费用辅助 | 明确假设的测算；数据落实后再增加费用导入与实绩复盘 | 测算金额精确；实耗可对账；缺消耗、缺成本、零分母、已超支时行为正确 |
| 4. 聊天前端与试用 | React/Vite聊天工作台、认证、日志、定时同步、备份恢复和演示说明 | 前端无手动查询页；刷新可恢复会话；桌面和窄屏可用；先用抖音试用一周并记录失败问法、耗时和模型成本 |

工期先按单人、熟悉 Python/React、数据权限可用的前提估算：可行性1～3个工作日，经营内核3～5日，FastAPI对话2～3日，React界面2～3日，费用与试用准备2～4日，合计约3周的初版工作量。第三方接口审批、特殊平台接入和对账问题另算；这不是已验证的交付承诺。
...
- API检查会话归属、同会话并发、SSE事件顺序和断线后重载；前端检查SSE分片解析、发送状态、错误恢复、空会话及窄屏布局。公开路由中不得出现通用指标查询接口。
